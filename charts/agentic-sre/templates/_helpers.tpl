...
Common labels.
*/}}
{{- define "srenix.labels" -}}
helm.sh/chart: {{ printf "%s-%s" .Chart.Name .Chart.Version | replace "+" "_" | trunc 63 | trimSuffix "-" }}
app.kubernetes.io/name: {{ include "srenix.name" . }}
...
ServiceAccount name.
*/}}
{{- define "srenix.serviceAccountName" -}}
{{- if .Values.serviceAccount.create -}}
{{- default (printf "%s-sa" (include "srenix.fullname" .)) .Values.serviceAccount.name -}}
{{- else -}}
...
*/}}
{{- define "srenix.readerName" -}}
{{- default (printf "%s-reader" (include "srenix.fullname" .)) .Values.rbac.reader.name -}}
{{- end -}}

{{/*
Remediator ClusterRole name.
*/}}
{{- define "srenix.remediatorName" -}}
{{- default (printf "%s-remediator" (include "srenix.fullname" .)) .Values.rbac.remediator.name -}}
{{- end -}}

{{/*
Operator (srenix-operator) workload name. Phase 1b — the controller-runtime
manager that reconciles AgenticSRE CRs.
*/}}
{{- define "srenix.operatorName" -}}
{{- printf "%s-operator" (include "srenix.fullname" .) -}}
{{- end -}}
...
{{- /*
srenix.aiImage — the commercial Srenix Enterprise image for the AI-companion
Deployment. Repository defaults to docker4zerocool/srenix-enterprise; tag
defaults to "v<AppVersion>" (srenix-enterprise images are tagged with a leading
"v", unlike the OSS image), so a chart at appVersion 1.8.2 pulls
docker4zerocool/srenix-enterprise:v1.8.2 — the Srenix Enterprise release pinned to the same
OSS engine. Override ai.image.tag to decouple.
*/ -}}
{{- define "srenix.aiImage" -}}
{{- $repo := (.Values.ai.image).repository | default "docker4zerocool/srenix-enterprise" -}}
...
{{- define "srenix.slackCriticalEnv" -}}
...
{{- define "srenix.slackHealthinfoEnv" -}}
...
{{- /*
srenix.protectedNamespacesEnv — APPEND-ONLY extension of the compiled-in
protected-namespace floor. Renders protectedNamespaces.extra as
SRENIX_PROTECTED_NAMESPACES_EXTRA (comma-separated). Consumed by BOTH
...
*/ -}}
{{- define "srenix.protectedNamespacesEnv" -}}
{{- with (.Values.protectedNamespaces).extra }}
- name: SRENIX_PROTECTED_NAMESPACES_EXTRA
  value: {{ join "," . | quote }}
{{- end }}
{{- end -}}

{{- /*
srenix.analyzerToggleEnv — env-var toggles for the v1.7+ drift-class
...
{{- /*
srenix.investigatorEnv — the layer-2 deterministic investigator defaults
ON (catalog.go opt-out via SRENIX_INVESTIGATOR=off). Flip
investigator.enabled=false to silence it.
*/ -}}
{{- define "srenix.investigatorEnv" -}}
{{- if (.Values.investigator) }}
{{- if not .Values.investigator.enabled }}
- name: SRENIX_INVESTIGATOR
  value: "off"
{{- end }}
{{- end }}
{{- end -}}
...
{{- /*
srenix.probeToggleEnv — env-var opt-outs for the v1.8 M2 probe-class
...
{{- /*
srenix.cloudProbeToggleEnv — env-var opt-outs for the per-cloud-probe
...
to emit SRENIX_CLOUD_PROBE_<PROVIDER>_<NAME>=off and skip that probe's
...
- name: SRENIX_CLOUD_PROBE_GCP_SUBNETS_SMALL_PREFIX
  value: {{ . | quote }}
{{- end }}
{{- $azure := (((.Values.cloud).azure).probes) | default dict }}
{{- if and (hasKey $azure "sql") (not $azure.sql) }}
- name: SRENIX_CLOUD_PROBE_AZURE_SQL
  value: "off"
{{- end }}
{{- if and (hasKey $azure "disks") (not $azure.disks) }}
- name: SRENIX_CLOUD_PROBE_AZURE_DISKS
  value: "off"
{{- end }}
{{- if and (hasKey $azure "aks") (not $azure.aks) }}
- name: SRENIX_CLOUD_PROBE_AZURE_AKS
  value: "off"
{{- end }}
{{- if and (hasKey $azure "identities") (not $azure.identities) }}
- name: SRENIX_CLOUD_PROBE_AZURE_IDENTITIES
  value: "off"
{{- end }}
{{- if and (hasKey $azure "subnets") (not $azure.subnets) }}
- name: SRENIX_CLOUD_PROBE_AZURE_SUBNETS
  value: "off"
{{- end }}
{{- if and (hasKey $azure "appgw") (not $azure.appgw) }}
- name: SRENIX_CLOUD_PROBE_AZURE_APPGW
  value: "off"
{{- end }}
{{- if and (hasKey $azure "certs") (not $azure.certs) }}
- name: SRENIX_CLOUD_PROBE_AZURE_CERTS
  value: "off"
{{- end }}
{{- if and (hasKey $azure "storage") (not $azure.storage) }}
- name: SRENIX_CLOUD_PROBE_AZURE_STORAGE
  value: "off"
{{- end }}
{{- if and (hasKey $azure "keyvaults") (not $azure.keyvaults) }}
- name: SRENIX_CLOUD_PROBE_AZURE_KEYVAULTS
  value: "off"
{{- end }}
{{- end -}}

{{- /*
srenix.externalDNSEnv — projects the Cloudflare API token for the
...
{{- /*
srenix.extraEnv — passthrough for arbitrary additional env vars on the
...
{{- /*
srenix.aiArgs — the `srenix-enterprise watch` flag surface for the AI-companion
Deployment (templates/aiwatch-deployment.yaml). These are the ONLY
flags the commercial `srenix-enterprise watch` binary accepts — it is the
...
diagnose/remediate CronJobs are NEVER swapped to srenix-enterprise; they keep
running the OSS image + operational loop. See docs/DEPLOYMENT.md.
*/ -}}
{{- define "srenix.aiArgs" -}}
- watch
...
- --memory-store-url={{ (.Values.ai.memory).storeUrl | default (printf "http://%s-rag.%s.svc:6333" (include "srenix.fullname" .) .Release.Namespace) }}
...
- --leader-election-name={{ include "srenix.fullname" . }}-aiwatch-leader
{{- end }}
...
{{- /* Phase 2.H — DigestPin PR attestation. Empty secretName = no
       ...
       mounts at /etc/srenix/attestation/ (not /etc/srenix/keys/) to
...
- --digest-pin-attestation-key=/etc/srenix/attestation/{{ (.Values.ai.digestPinAttestation).secretKey | default "attestation.key" }}
- --digest-pin-attestation-kid={{ (.Values.ai.digestPinAttestation).keyID | default "srenix-digest-pin" }}
{{- end }}
{{- end -}}

{{- /*
srenix.aiEnv — injects the LLM bearer token into the env var the Srenix Enterprise
...
{{- end }}
{{- include "srenix.ticketingProviderEnv" . }}
{{- end -}}

{{- /*
srenix.ticketingProviderEnv — renders the Srenix Enterprise Jira / ServiceNow / route
env vars on the aiwatch container. The srenix-enterprise binary consumes them; the
...
*/ -}}
{{- define "srenix.ticketingProviderEnv" -}}
{{- with .Values.ticketing }}
{{- with .route }}
- name: SRENIX_TICKETING_ROUTE
  value: {{ . | quote }}
{{- end }}
{{- with .jira }}
{{- with .url }}
- name: SRENIX_JIRA_URL
  value: {{ . | quote }}
{{- end }}
{{- with .project }}
- name: SRENIX_JIRA_PROJECT
  value: {{ . | quote }}
{{- end }}
{{- with .email }}
- name: SRENIX_JIRA_EMAIL
  value: {{ . | quote }}
{{- end }}
{{- with .issueType }}
- name: SRENIX_JIRA_ISSUE_TYPE
  value: {{ . | quote }}
{{- end }}
{{- with .priority }}
{{- with .critical }}
- name: SRENIX_JIRA_PRIORITY_CRITICAL
  value: {{ . | quote }}
{{- end }}
{{- with .warning }}
- name: SRENIX_JIRA_PRIORITY_WARNING
  value: {{ . | quote }}
{{- end }}
{{- with .info }}
- name: SRENIX_JIRA_PRIORITY_INFO
  value: {{ . | quote }}
{{- end }}
{{- end }}
{{- with .webUrlBase }}
- name: SRENIX_JIRA_WEB_URL_BASE
  value: {{ . | quote }}
{{- end }}
{{- if and .tokenSecret .tokenSecret.name .tokenSecret.key }}
- name: SRENIX_JIRA_TOKEN
  valueFrom:
    secretKeyRef:
...
{{- with .servicenow }}
{{- with .url }}
- name: SRENIX_SERVICENOW_URL
  value: {{ . | quote }}
{{- end }}
{{- with .user }}
- name: SRENIX_SERVICENOW_USER
  value: {{ . | quote }}
{{- end }}
{{- with .urgency }}
{{- with .critical }}
- name: SRENIX_SERVICENOW_URGENCY_CRITICAL
  value: {{ . | quote }}
{{- end }}
{{- with .warning }}
- name: SRENIX_SERVICENOW_URGENCY_WARNING
  value: {{ . | quote }}
{{- end }}
{{- with .info }}
- name: SRENIX_SERVICENOW_URGENCY_INFO
  value: {{ . | quote }}
{{- end }}
{{- end }}
{{- with .impact }}
{{- with .critical }}
- name: SRENIX_SERVICENOW_IMPACT_CRITICAL
  value: {{ . | quote }}
{{- end }}
{{- with .warning }}
- name: SRENIX_SERVICENOW_IMPACT_WARNING
  value: {{ . | quote }}
{{- end }}
{{- with .info }}
- name: SRENIX_SERVICENOW_IMPACT_INFO
  value: {{ . | quote }}
{{- end }}
{{- end }}
{{- with .webUrlBase }}
- name: SRENIX_SERVICENOW_WEB_URL_BASE
  value: {{ . | quote }}
{{- end }}
{{- if and .passwordSecret .passwordSecret.name .passwordSecret.key }}
- name: SRENIX_SERVICENOW_PASSWORD
  valueFrom:
    secretKeyRef:
