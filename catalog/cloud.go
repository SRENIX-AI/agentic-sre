...
import (
	...
	awsprobes "github.com/srenix-ai/agentic-sre/internal/cloud/aws"
	azureprobes "github.com/srenix-ai/agentic-sre/internal/cloud/azure"
...
// SRENIX_CLOUD_PROBE_<PROVIDER>_<NAME>=off (default ON) — the same opt-out
// pattern as the K8s SRENIX_PROBE_* gates. The chart's
// cloud.<provider>.probes.* values render these envs (see
// srenix.cloudProbeToggleEnv in _helpers.tpl). The control-plane toggles
...
// SRENIX_CLOUD_PROBE_GCP_SUBNETS_SMALL_PREFIX / the chart's
...
// next to the srenix.cloudProbeToggleEnv opt-outs, NOT a registration
// gate). Unset, non-numeric, or non-positive values return 0 = the
// probe's compiled-in default (/26).
func gcpSubnetSmallPrefix() int {
	raw := os.Getenv("SRENIX_CLOUD_PROBE_GCP_SUBNETS_SMALL_PREFIX")
	if raw == "" {
		return 0
	}
	n, err := strconv.Atoi(raw)
	if err != nil || n <= 0 {
		log.Printf("catalog: invalid SRENIX_CLOUD_PROBE_GCP_SUBNETS_SMALL_PREFIX=%q (want a positive integer prefix length); using the probe's compiled-in /26 default", raw)
		return 0
	}
	return n
}
