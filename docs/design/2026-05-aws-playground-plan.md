...
### Drift-injector CronJob (4-scenario rotation, every 30s)
  ...
  1. Create a Pod with a deliberately bad image-pull secret → CHA emits an
     `ImagePullAuth` DriftReport → CHA fixer re-syncs the ExternalSecret →
     resolved.
  2. Trigger LocalStack RDS `StorageFull` event → CHA cloud-probe emits a
     DriftReport → static "ticketed to OpenProject" message (no cloud
     mutation fixer in OSS).
  3. Patch a Service annotation to an invalid TLS secret name → CHA
     `TLSSecretMismatch` fixer kicks in → resolved.
  4. Delete a Job pod mid-run → CHA `StuckJob` fixer cleans it up.
...
## Implementation phases (~6 days end-to-end)
  ...
  | **2** | EC2 bootstrap: Docker + kind + LocalStack + CHA helm install + bootstrap script (idempotent, reruns clean) | 1d | — |
  ...
  | **6** | Replace `/playground/` `StubBody` in `cha-website` with an iframe of the live viewer; cutover; verify live | 0.5d | — |

## Risks & mitigations

  - **R1 — `t3.small` memory pressure** (2 GiB w/ Docker + kind + LocalStack
    + CHA). Mitigation: LocalStack community-edition lite mode (skip heavy
    services like S3 multipart); cap CHA's watcher resync to 5 minutes;
    profile actual usage on day 1 — if memory utilisation spikes > 70%,
    upgrade to `t3.medium` (~$30/mo).
  - **R2 — LocalStack community coverage gaps**. The 5 services CHA's AWS
...
## Out of scope (deliberately)
  ...
  - **Per-visitor ephemeral cluster.** Cost blow-up; visitors don't expect
    to mutate a demo.
  - **CHA-com paid features in the playground.** No LLM call out — that
...
## Acceptance criteria
  ...
  - Helm install CTA on the iframe redirects to the OSS repo `README.md`
    install section.
  - `/playground/` on the cha-website iframes the live viewer (replaces
    the `StubBody`).
