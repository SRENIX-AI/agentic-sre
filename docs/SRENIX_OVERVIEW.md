# Agentic SRE — Two-Page Overview

*A read-it-once explainer of what Srenix is, where the line between OSS and Paid
...
Agentic SRE (`srenix`) is a **self-healing operational layer** for
...
3. **Investigates** (v1.5+) — on every CRITICAL finding, a read-only
   ...
   In Srenix Enterprise with `ai.enabled=true`, the deep-RCA investigator additionally
   ...
   analysis to the `srenix_investigations` Qdrant collection; that analysis is
...
- **Zero-trust offline** — `srenix diagnose --snapshot ./your-export/` against a
...
Srenix is **open-core**. The engine and the foundational catalog are Apache 2.0
in the public repo. The paid tier ships as a separate signed binary
(`srenix-enterprise`) that imports the OSS module, registers additional patterns
...
## What Srenix does — and what it deliberately doesn't do
...
- **Patch GitOps-managed Ingresses.** If an Ingress carries an ArgoCD,
  ...
  fixer skips it — the right fix lives in the source repo, and Srenix refuses
...
- **Edit your manifests in git.** The catalog produces precise remediation
  ...
  them in the source repo. Srenix does not have a git-write surface.
...
- **Send cluster state to any third-party SaaS by default.** Outbound traffic
  ...
  **One deliberate exception (Srenix Enterprise paid, opt-in):** when
...
Srenix has two distinct AI surfaces, in increasing order of trust required:
...
- **Layer-2 Investigator (Paid, LLM-backed deep-RCA)** — same `Investigator`
  ...
  runbooks, and persists the root-cause analysis to the `srenix_investigations`
...
- **T3 Vault Runbook Proposer** — generates a `vault kv patch` command
  ...
  execution under dual approval. Srenix Enterprise never executes Vault writes.
...
Srenix detects the configuration-drift failure class that metric dashboards
