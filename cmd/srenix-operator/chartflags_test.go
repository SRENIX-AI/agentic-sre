...
func TestOperatorChartFlags_MatchBinaryFlagSet(t *testing.T) {
	...
	if len(unknown) > 0 {
		...
		t.Errorf("chart renders --%s on the operator container, but srenix-operator does NOT register it — v1.23.0 CrashLoop class (pod fails with \"flag provided but not defined\"). Register the flag in registerOperatorFlags() or fix the template.",
