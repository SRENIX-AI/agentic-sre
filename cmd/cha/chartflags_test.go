...
// cmd/cha-operator/chartflags_test.go (separate main package).

// chaSubcommandFlags maps the workload role label to the cha subcommand
...
func TestChartFlags_MatchBinaryFlagSet(t *testing.T) {
	...
	for role, sub := range chaSubcommandRole {
		...
		if cmd == nil {
			t.Fatalf("cha has no %q subcommand — newRootCmd() shape changed", sub)
		}
		...
		if len(unknown) > 0 {
			...
			t.Errorf("chart renders --%s on the %q container, but `cha %s` does NOT register %s — this is the v1.23.0 CrashLoop class (the pod fails with \"unknown flag\" at startup). Register the flag on the cobra command or fix the template.",
