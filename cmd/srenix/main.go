...
// srenix — Agentic SRE CLI
//
// Subcommands:
//
//	srenix diagnose --snapshot <path>   # zero-trust offline mode (no cluster access)
//	srenix diagnose --live              # live cluster mode (uses kubeconfig or in-cluster SA)
//	srenix snapshot capture             # wraps `kubectl get` into a kubectl-shaped JSON bundle
//	srenix remediate --live             # runs the whitelisted auto-fixers (live mode only)
//	srenix anonymize [file...]          # anonymize srenix diagnose --format json output to JSONL
//	srenix version                      # version info
package main
...
import (
	...
	"github.com/srenix-ai/agentic-sre/catalog"
	"github.com/srenix-ai/agentic-sre/internal/anonymize"
	cloudimpl "github.com/srenix-ai/agentic-sre/internal/cloud"
	awsimpl "github.com/srenix-ai/agentic-sre/internal/cloud/aws"
	azureimpl "github.com/srenix-ai/agentic-sre/internal/cloud/azure"
	gcpimpl "github.com/srenix-ai/agentic-sre/internal/cloud/gcp"
	...
	"github.com/srenix-ai/agentic-sre/internal/summarize"
	"github.com/srenix-ai/agentic-sre/internal/vault"
	"github.com/srenix-ai/agentic-sre/internal/watcher"
	"github.com/srenix-ai/agentic-sre/pkg/ai"
	cloudpkg "github.com/srenix-ai/agentic-sre/pkg/cloud"
	cloudpkgaws "github.com/srenix-ai/agentic-sre/pkg/cloud/aws"
	cloudpkgazure "github.com/srenix-ai/agentic-sre/pkg/cloud/azure"
	cloudpkggcp "github.com/srenix-ai/agentic-sre/pkg/cloud/gcp"
	"github.com/srenix-ai/agentic-sre/pkg/silence"
	"github.com/srenix-ai/agentic-sre/pkg/ticketing/openproject"
...
// then default loading rules). Kept local so cmd/srenix doesn't depend
...
// newRootCmd builds the full `srenix` cobra command tree (root +
// subcommands). Factored out of main() so the chart-args↔binary-flags
// parity gate (cmd/srenix/chartflags_test.go) can introspect the real
// FlagSets instead of a hand-maintained list — keeping the gate's
// notion of "valid flags" in lockstep with the binary.
func newRootCmd() *cobra.Command {
	root := &cobra.Command{
		Use:   "srenix",
		Short: "Agentic SRE — detect, fix, re-verify, report.",
		Long: `srenix runs a battery of probes against your Kubernetes cluster, applies
...
func snapshotCaptureCmd() *cobra.Command {
	...
	c := &cobra.Command{
		Use:   "capture",
		Short: "Capture cluster state into a directory or tarball for `srenix diagnose --snapshot`",
		Long: `Captures the canonical resource set required by srenix diagnose offline:
...
shape so the same files round-trip back through srenix diagnose --snapshot.`,
		Example: `  # Capture into a directory
  srenix snapshot capture --out ./snapshot

  # Capture into a tarball (single artifact, easy to share)
  srenix snapshot capture --tar snapshot.tgz

  # Then diagnose offline
  srenix diagnose --snapshot ./snapshot`,
...
Secrets, ConfigMaps, or any CRD.`,
		Example: `  srenix remediate --live
  srenix remediate --live --format json
  srenix remediate --live --dry-run`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			if !live {
...
func printRemediateText(results []fix.Result, dryRun bool) {
	...
	fmt.Printf("Agentic SRE — remediate (%s)\n", tag)
	fmt.Println(repeatRune('=', 60))
	totalActions := 0
	totalSkipped := 0
...
// anonymizeCmd implements `srenix anonymize [file...]`.
// Each input file must be a `srenix diagnose --format json` run. When no files
...
func anonymizeCmd() *cobra.Command {
	...
	c := &cobra.Command{
		Use:   "anonymize [file...]",
		Short: "Anonymize srenix diagnose --format json output to JSONL",
		Long: `anonymize reads one or more srenix diagnose --format json files (or stdin)
...
Typical pipeline (daily cron + nightly GitHub Action):
  ...
  srenix diagnose --live --format json > /tmp/run.json
  srenix anonymize /tmp/run.json >> runs/$(date +%Y-%m-%d).jsonl`,
...
// summarizeCmd implements `srenix summarize <runs-dir>`.
...
Typical usage (nightly GitHub Action):
  ...
  srenix summarize runs/ > runs/SUMMARY.md`,
...
func watchCmd() *cobra.Command {
	var (
		...
		// only in srenix-enterprise's aiwatch process, which posted URLs to
	...
	c := &cobra.Command{
		...
		Long: `Starts Kubernetes watches for all resource types srenix analyzes.

On relevant create/update/delete events a short debounce fires, then the
full probe+analyzer stack runs — identical to srenix diagnose --live.
...
the post-fix cluster state.`,
  ...
  srenix watch --live --slack-webhook=$(SLACK_WEBHOOK_URL) --remedy

  # Tune debounce and repeat interval
  srenix watch --live --debounce=15s --slack-repeat-interval=6h \
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if approvalServerURL != "" {
				...
				if kerr != nil {
					...
					// This keeps srenix-enterprise (which registers its own
			...
			// through.
			leaderDisabled := os.Getenv("SRENIX_LEADER_ELECTION") == "off" ||
				os.Getenv("SRENIX_LEADER_ELECTION") == "false" ||
				os.Getenv("SRENIX_LEADER_ELECTION") == "0"
	...
	c.Flags().StringVar(&alertmanagerURL, "alertmanager-url", os.Getenv("ALERTMANAGER_URL"), "Alertmanager base URL (e.g. http://alertmanager.pg:9093). When set, Srenix posts all active issues as Prometheus alerts each cycle; AM handles routing to Slack/PagerDuty/etc.")
	...
	c.Flags().StringSliceVar(&promTriggerFilter, "prom-trigger-alert-filter", nil,
		"Comma-separated alertname filter (case-insensitive). Empty = any firing alert triggers; non-empty = only the listed alertnames trigger. Useful when the cluster has high-volume scrape-noise alerts that shouldn't churn Srenix.")
	// M6 — webhook class-E receiver (v1.23.0+).
	c.Flags().StringVar(&webhookListen, "webhook-listen", os.Getenv("WEBHOOK_LISTEN"),
		"Listen address for the HMAC-authed webhook trigger receiver (e.g. ':8090'). Empty = receiver disabled. Each registered --webhook-source produces an endpoint at /webhook/<source>; POSTing a body with X-Srenix-Signature=sha256=<hex-hmac-sha256-of-body> triggers an immediate diagnose cycle. Senders SHOULD also send X-Srenix-Timestamp=<unix-seconds> and sign 'timestamp.body' instead (X-Srenix-Signature=sha256=hex(hmac-sha256(secret, timestamp+\".\"+body))); timestamped requests more than 5 minutes from server time are rejected with 401, bounding replay of captured requests. Requests without the timestamp header keep the legacy body-only check. GET /webhook/health returns 200 for K8s liveness probes.")
	c.Flags().StringSliceVar(&webhookSourceSpec, "webhook-source", nil,
		"Repeatable webhook source registration in the form '<name>=<env-var-with-hmac-secret>'. Example: --webhook-source=vault=SRENIX_WEBHOOK_VAULT_SECRET. FAIL-CLOSED: if the env var is unset/empty, or the spec is malformed, an error is logged and the source is NOT registered. To run a deliberately unauthenticated source use the literal token '<name>=insecure-no-hmac' (DANGEROUS: anyone who can reach the listener can trigger diagnose cycles).")
	...
	c.Flags().StringVar(&slackAlerts, "slack-alerts", "", "Slack webhook for #ceph-alerts — Srenix acted (auto-fixed issues); used as fallback when --alertmanager-url is not set")
	...
	c.Flags().StringVar(&ticketingProvider, "ticketing-provider", os.Getenv("TICKETING_PROVIDER"), "Issue tracker sink: 'openproject' (OSS). Empty disables ticketing. Jira/ServiceNow live in Srenix Enterprise.")
	...
	c.Flags().StringVar(&ticketingPriorityCrit, "ticketing-priority-critical", os.Getenv("TICKETING_PRIORITY_CRITICAL"), "OpenProject priority ID for Srenix severity=critical (e.g. '75' for Immediate)")
	c.Flags().StringVar(&ticketingPriorityWarn, "ticketing-priority-warning", os.Getenv("TICKETING_PRIORITY_WARNING"), "OpenProject priority ID for Srenix severity=warning (e.g. '74' for High)")
	c.Flags().StringVar(&ticketingPriorityInfo, "ticketing-priority-info", os.Getenv("TICKETING_PRIORITY_INFO"), "OpenProject priority ID for Srenix severity=info (e.g. '73' for Normal)")
	...
	c.Flags().StringSliceVar(&ticketingLabels, "ticketing-labels", []string{"srenix", "auto-filed"}, "Labels appended to ticket descriptions for filtering")
	...
	c.Flags().StringVar(&approvalServerURL, "approval-server-url", "",
		"Base URL of the approval-server (e.g. https://srenix-approve.example.com). When set AND --signing-key-path resolves to a valid Ed25519 key, the watcher registers an ai.ManifestBridge FixProposer + signer and mints signed approve/deny URLs that ride through Slack / Alertmanager / DriftReport / ticketing adapters. Empty = no URL minting (pre-v1.16.0 behavior).")
	c.Flags().StringVar(&signingKeyPath, "signing-key-path", os.Getenv(ai.EnvSigningKeyPath),
		"Path to the Ed25519 signing key file (base64-encoded raw bytes). Defaults to $SRENIX_SIGNING_KEY_PATH or "+ai.DefaultSigningKeyPath+". Required when --approval-server-url is set; missing key falls back to URL-less mode (text-only proposal, no click-to-fix).")
...
// "openproject"; Jira and ServiceNow plug in here in Srenix Enterprise.
//
...
func diagnoseCmd() *cobra.Command {
	...
	c.Flags().StringVar(&silenceApprovalURL, "approval-server-url", "", "Approval-server external base URL (e.g. https://srenix-approve.example.com). When set with --signing-key-path, --slack-webhook posts include signed 🔕 one-click Silence links under each critical finding.")
	c.Flags().StringVar(&silenceSigningKey, "signing-key-path", "", "Path to the Ed25519 signing key (default: $SRENIX_SIGNING_KEY_PATH then "+ai.DefaultSigningKeyPath+"). Required for one-click Silence links.")
...
func printText(results []probe.Result, diagnostics []diagnose.Diagnostic, mode snapshot.Mode) error {
	fmt.Printf("Agentic SRE — diagnose (%s mode)\n", mode)
	fmt.Println(repeatRune('=', 60))
	totalFindings := 0
	for _, r := range results {
