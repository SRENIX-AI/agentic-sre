...
// read role must reference it via the `cha.serviceAccountName` helper — the
// actual SA the watcher Deployment runs as (`<fullname>-sa`). A binding that
// uses `cha.fullname` (no `-sa`) points at a non-existent SA, so the grant
...
// excludes the cha.bionicaisolutions.com group. This file is the cheap guard.
func TestChartWatcherBindings_UseServiceAccountHelper(t *testing.T) {
	...
	dir := filepath.Join("..", "..", "charts", "cluster-health-autopilot", "templates")
	// Subject SA-name line inside a `kind: ServiceAccount` block.
	saName := regexp.MustCompile(`name:\s*\{\{\s*include\s+"cha\.(\w+)"`)
	for _, f := range watcherBindingFiles {
		...
		if m[1] != "serviceAccountName" {
			t.Errorf("%s: watcher binding references the SA via cha.%s, but the watcher runs as cha.serviceAccountName (<fullname>-sa). "+
				"cha.fullname points at a non-existent SA, so the grant silently never reaches the watcher (live symptom: \"... is forbidden\"). "+
				"Use {{ include \"cha.serviceAccountName\" . }}.", f, m[1])
		}
	}
}
