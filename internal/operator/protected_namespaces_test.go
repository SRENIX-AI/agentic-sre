...
// spec.protectedNamespacesExtra → SRENIX_PROTECTED_NAMESPACES_EXTRA on
...
// (pkg/ai.IsProtectedNamespace linked into srenix-enterprise).
...
func protectedCR() *chav1alpha1.AgenticSRE {
	cr := sampleCR()
...
func TestProtectedNamespacesExtra_WatcherEnv(t *testing.T) {
	...
	got, ok := envValue(d.Spec.Template.Spec.Containers[0].Env, "SRENIX_PROTECTED_NAMESPACES_EXTRA")
	if !ok {
		t.Fatal("watcher env missing SRENIX_PROTECTED_NAMESPACES_EXTRA")
	}
	if got != "prod-payments,tenant-a" {
		t.Errorf("watcher SRENIX_PROTECTED_NAMESPACES_EXTRA = %q; want trimmed+deduped %q", got, "prod-payments,tenant-a")
	}
}
...
func TestProtectedNamespacesExtra_DiagnoseAndRemediateEnv(t *testing.T) {
	...
	for name, env := range map[string][]corev1.EnvVar{
		...
		got, ok := envValue(env, "SRENIX_PROTECTED_NAMESPACES_EXTRA")
		if !ok {
			t.Errorf("%s env missing SRENIX_PROTECTED_NAMESPACES_EXTRA", name)
			continue
		}
		if got != "prod-payments,tenant-a" {
			t.Errorf("%s SRENIX_PROTECTED_NAMESPACES_EXTRA = %q; want %q", name, got, "prod-payments,tenant-a")
		}
	}
}
...
func TestProtectedNamespacesExtra_AIWatchEnv(t *testing.T) {
	...
	got, ok := envValue(d.Spec.Template.Spec.Containers[0].Env, "SRENIX_PROTECTED_NAMESPACES_EXTRA")
	if !ok {
		t.Fatal("aiwatch env missing SRENIX_PROTECTED_NAMESPACES_EXTRA — the srenix-enterprise AI validator floor would diverge from the fixer guard")
	}
	if got != "prod-payments,tenant-a" {
		t.Errorf("aiwatch SRENIX_PROTECTED_NAMESPACES_EXTRA = %q; want %q", got, "prod-payments,tenant-a")
	}
}
...
func TestProtectedNamespacesExtra_AbsentWhenUnset(t *testing.T) {
	...
	for name, env := range map[string][]corev1.EnvVar{
		...
		if v, ok := envValue(env, "SRENIX_PROTECTED_NAMESPACES_EXTRA"); ok {
			t.Errorf("%s renders SRENIX_PROTECTED_NAMESPACES_EXTRA=%q with no extras set; want absent (byte-identical legacy render)", name, v)
		}
	}
}
...
func TestProtectedNamespacesExtra_AllGarbageRendersNothing(t *testing.T) {
	...
	cr.Spec.Watcher = &chav1alpha1.WatcherSpec{Enabled: true}
	d := BuildWatcherDeployment(cr)
	if v, ok := envValue(d.Spec.Template.Spec.Containers[0].Env, "SRENIX_PROTECTED_NAMESPACES_EXTRA"); ok {
		t.Errorf("all-garbage extras rendered SRENIX_PROTECTED_NAMESPACES_EXTRA=%q; want absent", v)
	}
}
