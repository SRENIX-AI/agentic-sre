...
func (c CapacityDrift) checkPVCExpansion(ctx context.Context, src snapshot.Source, now time.Time, grace time.Duration) []Diagnostic {
	...
	for i := range list.Items {
		...
		out = append(out, Diagnostic{
			...
			Remediation: "The PVC was resized but the storage class doesn't support volume expansion, or the CSI driver hasn't completed it. " +
				"Verify `kubectl get storageclass <name> -o jsonpath='{.allowVolumeExpansion}'` returns `true`. If false, expansion is impossible — " +
				"create a new PVC at the larger size and migrate. If true, inspect the CSI driver controller logs.",
		})
	}
	return out
}
